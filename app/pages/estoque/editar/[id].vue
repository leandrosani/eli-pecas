<template>
  <div class="max-w-4xl mx-auto pb-20">
    ...
    <div v-if="status === 'pending'" class="text-center py-20">Carregando...</div>
    ...
    <form v-else @submit.prevent="salvar" class="space-y-6">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="p-6 pt-0">
          <div class="space-y-2">
            ...
            <textarea v-model="form.detalhes" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-black focus:border-black" rows="3"></textarea>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center gap-3">
          ...
          <h2 class="font-semibold text-gray-900">Valores</h2>
        </div>
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
...
const form = reactive({
  nome: '', marca: '', modelo: '', ano: undefined, preco: undefined, quantidade: 1, estado: '', detalhes: ''
})
